...
import { login, apiAdmin, apiClient } from '../../src/apiClient';
import { generateCreateProfilePayload, generateEditProfilePayload, fakeData, generateResetPasswordPayloads, generateSetNewPasswordPayloads } from '../../src/utils/payloads';
import { assertDeoProfileCreateEditDelete, assertDeoProfileProperty, assertResetSetNewPassword } from '../../src/utils/assertions';
import dotenv from 'dotenv';
import axios from 'axios';
import Database from '../../src/utils/dbData';
import util from 'util';
...
describe('API Tests', () => {
    ...
    it('Create Profile', async () => {
        ...
        try {
            const response = await apiAdmin.post('/admin/profileCreation/createProfile', createProfilePayload);
            assertDeoProfileCreateEditDelete(response, 'Account created');
    ...
    it('Set New Password', async () => {
        ...
        try {
            const response = await apiClient.post('/api/users/reset-password', setPasswordPayloads);
            assertResetSetNewPassword(response, "Password Changed Successfully");
        } catch (error) {
            throw error;
        }
    }, 20000);
    ...
    it('Update DEO Profile', async () => {
        ...
        try {
            const response = await apiAdmin.put('/admin/profileCreation/updateProfile', updateProfilePayloads);
            assertDeoProfileCreateEditDelete(response, 'Account Updated');
        } catch (error) {
            throw error;
        }
    }, 20000);
    ...
    it('Delete DEO Profile', async () => {
        const deletePayload = { id: profileId };

        try {
            const response = await apiAdmin.delete('/admin/profileCreation/deleteDEOProfile', { data: deletePayload });
            assertDeoProfileCreateEditDelete(response, "Account deleted");
        } catch (error) {
            if (axios.isAxiosError(error)) {
                console.log(util.inspect(error.response?.data, { depth: null, colors: true }));
            } else {
